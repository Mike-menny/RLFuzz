...
class CountdownORM(ORM):
    def __call__(self, completions,**kwargs) -> List[float]:
        ...
        for i in range(len(completions)):
            ...
            with open(f"/workspace/output/projects/{project_name}/data.json", 'r', encoding='utf-8') as f:
                data = json.load(f)
    
            dependency = Reward.dependency_check(api_list=API_Called[1:], context = data['context'])
            rewards.append(0.04 + API_Called[0] + dependency)
